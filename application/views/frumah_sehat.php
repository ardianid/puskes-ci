...
$(document).ready(function() {
     ...
     $("#tnama_desa").autocomplete({  
                ...
                function(req, add){  
                    $.ajax({  
                        url: "rumah_sehat_ctrl/lookup_desa",  
                        dataType: 'json',  
                        type: 'POST',  
                        data: req,  
                        success:      
      ...
      $("#tnama_petugas").autocomplete({  
                ...
                function(req, add){  
                    $.ajax({  
                        url: "rumah_sehat_ctrl/lookup_petugas",  
                        dataType: 'json',  
                        type: 'POST',  
                        data: req,  
                        success:      
  ...
  table_pencarian = $('#table_cari').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('rumah_sehat_ctrl/ajax_list_pencarian')?>",
            "type": "POST",
            "data": function(d) {
              d.nama_rmah_sht= $('#tnama_rmah_cr').val();
              d.nama_desa= $('#nama_desa_cr').val();
        ...
        "columnDefs": [
          { width: '70px', targets: 0,"orderable": false },
...
function edit_ins(id)
{
  ...
  $.ajax({
        url : "<?php echo site_url('rumah_sehat_ctrl/ajax_get_by_id_pencarian')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

      $('[name="tid_inspeksi"]').val(data.id_inspeksi_rsehat);

      $('[name="ttanggal"]').val(data.tgl_ins_rsehat);
      $('[name="tnama_kk"]').val(data.nama_kk);
      ...
      $('[name="tjml_jiwa"]').val(data.jml_jiwa);
...
function delete_ins(id)
    {
      if(confirm('Yakin akan dihapus?'))
      {
        // ajax delete data to database
          $.ajax({
            url : "<?php echo site_url('rumah_sehat_ctrl/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    function save()
    {
      ...
      if(save_method == 'add')
      {
          url = "<?php echo site_url('rumah_sehat_ctrl/ajax_add')?>";
      }
      else
      {
        url = "<?php echo site_url('rumah_sehat_ctrl/ajax_update')?>";
      }
       ...
       // ajax adding data to database
          $.ajax({
            ...
            type: "POST",
            data: {
              "tgl_ins_rsehat": $("#ttanggal").val(),
              "kd_desa": $("#tkd_desa").val(),
              "nama_kk": $("#tnama_kk").val(),
              "rt": $("#trt").val(),
              "rw": $("#trw").val(),
              "kd_petugas": $("#tkd_petugas").val(),
              "jml_jiwa": $("#tjml_jiwa").val(),
              "tot_nilai": $("#ttot_nilai").val(),
              "kd_puskes": $("#tkd_puskes").val(),
              "id_inspeksi_rsehat": $("#tid_inspeksi").val(),
            } ,
            dataType: "JSON",
            success: function(data)
            {
  ...
  <!-- menu tab -->
      <div class="panel with-nav-tabs panel-primary">
            ...
            <div class="alert alert-warning">
              ...
              <tr>
                <td width="15%">Nama KK</td>
                <td width="1%">:</td>
                <td width="25%">
                  <input type="text" class="form-control input-sm" name="tnama_rmah_cr" id="tnama_rmah_cr" >
                </td>
                <td width="7%">&nbsp;</td>
              ...
              <tr>
                ...
                <td>:</td>
                <td>
                  <input type="text" class="form-control input-sm" name="nama_desa_cr" id="nama_desa_cr" >
                </td>
                <td>&nbsp;</td>
                <td></td>
                <td></td>
       ...
       <div class="tab-pane fade" id="tab_detail">
          ...
          <!--  <div class="alert alert-warning"> -->
            ...
            <tr>
              ...
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm" id="tnama_kk" name="tnama_kk">
              </td>
            </tr>
            <tr>
              <td>RT</td>
            ...
            <tr>
              ...
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm size_kecil" id="tjml_jiwa" name="tjml_jiwa">
              </td>
            </tr>

...
$(document).ready(function() {
     ...
     $("#tnama_desa").autocomplete({  
                ...
                function(req, add){  
                    $.ajax({  
                        url: "inspeksi_htl_ctrl/lookup_desa",  
                        dataType: 'json',  
                        type: 'POST',  
                        data: req,  
                        success:      
      ...
      $("#tnama_petugas").autocomplete({  
                ...
                function(req, add){  
                    $.ajax({  
                        url: "inspeksi_htl_ctrl/lookup_petugas",  
                        dataType: 'json',  
                        type: 'POST',  
                        data: req,  
                        success:      
  ...
  table_pencarian = $('#table_cari').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('inspeksi_htl_ctrl/ajax_list_pencarian')?>",
            "type": "POST",
            "data": function(d) {
              d.nama_htl= $('#tnama_htl_cr').val();
              d.alamat_htl= $('#talamat_htl_cr').val();
...
function kosongkan_data(){
  ...
  $('[name="tnama_htl"]').val('');
  $('[name="talamat_htl"]').val('');
  $('[name="tnotelp_htl"]').val('');
  $('[name="tnoijin_htl"]').val('');
...
function edit_ins(id)
{
  ...
  $.ajax({
        url : "<?php echo site_url('inspeksi_htl_ctrl/ajax_get_by_id_pencarian')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

      $('[name="tid_inspeksi"]').val(data.id_inspeksi_htl);

      $('[name="ttanggal"]').val(data.tgl_ins_htl);
      $('[name="tnama_htl"]').val(data.nama_htl);
      $('[name="talamat_htl"]').val(data.alamat_htl);
      $('[name="tnotelp_htl"]').val(data.notelp_htl);
      $('[name="tnoijin_htl"]').val(data.noijin_htl);
      ...
      $('[name="tpenanggung"]').val(data.penanggung_htl);
      $('[name="tjml_kary"]').val(data.jml_kary);
...
function delete_ins(id)
    {
      if(confirm('Yakin akan dihapus?'))
      {
        // ajax delete data to database
          $.ajax({
            url : "<?php echo site_url('inspeksi_htl_ctrl/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    function save()
    {
      ...
      if(save_method == 'add')
      {
          url = "<?php echo site_url('inspeksi_htl_ctrl/ajax_add')?>";
      }
      else
      {
        url = "<?php echo site_url('inspeksi_htl_ctrl/ajax_update')?>";
      }
       ...
       // ajax adding data to database
          $.ajax({
            ...
            type: "POST",
            data: {
              "tgl_ins_htl": $("#ttanggal").val(),
              "kd_desa": $("#tkd_desa").val(),
              "nama_htl": $("#tnama_htl").val(),
              "alamat_htl": $("#talamat_htl").val(),
              "notelp_htl": $("#tnotelp_htl").val(),
              "noijin_htl": $("#tnoijin_htl").val(),
              "kd_petugas": $("#tkd_petugas").val(),
              "penanggung_htl": $("#tpenanggung").val(),
              "jml_kary": $("#tjml_kary").val(),
              "tot_nilai": $("#ttot_nilai").val(),
              "kd_puskes": $("#tkd_puskes").val(),
              "id_inspeksi_htl": $("#tid_inspeksi").val(),
            } ,
            dataType: "JSON",
            success: function(data)
            {
  ...
  <!-- menu tab -->
      <div class="panel with-nav-tabs panel-primary">
            ...
            <div class="alert alert-warning">
              ...
              <tr>
                <td width="15%">Nama Hotel</td>
                <td width="1%">:</td>
                <td width="25%">
                  <input type="text" class="form-control input-sm" name="tnama_htl_cr" id="tnama_htl_cr" >
                </td>
                <td width="7%">&nbsp;</td>
              ...
              <tr>
                ...
                <td>:</td>
                <td>
                  <input type="text" class="form-control input-sm" name="talamat_htl_cr" id="talamat_htl_cr" >
                </td>
                <td>&nbsp;</td>
                <td></td>
                <td></td>
       ...
       <div class="tab-pane fade" id="tab_detail">
          ...
          <!--  <div class="alert alert-warning"> -->
            ...
            <tr>
              ...
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm" id="tnama_htl" name="tnama_htl">
              </td>
            </tr>
            <tr>
              <td>Alamat</td>
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm" id="talamat_htl" name="talamat_htl">
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm" id="tnotelp_htl" name="tnotelp_htl">
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm" id="tnoijin_htl" name="tnoijin_htl">
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm size_kecil" id="tjml_kary" name="tjml_kary">
              </td>
            </tr>

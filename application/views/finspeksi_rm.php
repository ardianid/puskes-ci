...
<head>
    ...
    <title>Inspeksi RM dan Restoran</title>
...
$(document).ready(function() {
     ...
     $("#tnama_desa").autocomplete({  
                ...
                function(req, add){  
                    $.ajax({  
                        url: "inspeksi_rm_ctrl/lookup_desa",  
                        dataType: 'json',  
                        type: 'POST',  
                        data: req,  
                        success:      
      ...
      $("#tnama_petugas").autocomplete({  
                ...
                function(req, add){  
                    $.ajax({  
                        url: "inspeksi_rm_ctrl/lookup_petugas",  
                        dataType: 'json',  
                        type: 'POST',  
                        data: req,  
                        success:      
  ...
  table_pencarian = $('#table_cari').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('inspeksi_rm_ctrl/ajax_list_pencarian')?>",
            "type": "POST",
            "data": function(d) {
              d.nama_rm= $('#tnama_rm_cr').val();
              d.alamat_rm= $('#talamat_rm_cr').val();
...
function edit_ins(id)
{
  ...
  $.ajax({
        url : "<?php echo site_url('inspeksi_rm_ctrl/ajax_get_by_id_pencarian')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

      $('[name="tid_inspeksi"]').val(data.id_inspeksi);

      $('[name="ttanggal"]').val(data.tgl_ins);
      $('[name="tnama_rm"]').val(data.nama_rm);
      $('[name="talamat_rm"]').val(data.alamat_rm);
      ...
      $('[name="tpenanggung"]').val(data.penanggung);
      $('[name="tjml_kary"]').val(data.jml_kary);
      $('[name="tjml_penj"]').val(data.jml_penj);
      $('[name="tno_ijin"]').val(data.no_ijin);
...
function delete_ins(id)
    {
      if(confirm('Yakin akan dihapus?'))
      {
        // ajax delete data to database
          $.ajax({
            url : "<?php echo site_url('inspeksi_rm_ctrl/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    function save()
    {
      ...
      if(save_method == 'add')
      {
          url = "<?php echo site_url('inspeksi_rm_ctrl/ajax_add')?>";
      }
      else
      {
        url = "<?php echo site_url('inspeksi_rm_ctrl/ajax_update')?>";
      }
       ...
       // ajax adding data to database
          $.ajax({
            ...
            type: "POST",
            data: {
              "tgl_ins": $("#ttanggal").val(),
              "kd_desa": $("#tkd_desa").val(),
              "nama_rm": $("#tnama_rm").val(),
              "alamat_rm": $("#talamat_rm").val(),
              "kd_petugas": $("#tkd_petugas").val(),
              "penanggung": $("#tpenanggung").val(),
              "jml_kary": $("#tjml_kary").val(),
              "jml_penj": $("#tjml_penj").val(),
              "no_ijin": $("#tno_ijin").val(),
              "tot_nilai": $("#ttot_nilai").val(),
              "kd_puskes": $("#tkd_puskes").val(),
              "id_inspeksi": $("#tid_inspeksi").val(),
            } ,
            dataType: "JSON",
            success: function(data)
            {
      ...
      <div class="panel with-nav-tabs panel-primary">
            ...
            <div class="alert alert-warning">
              ...
              <tr>
                ...
                <td width="1%">:</td>
                <td width="25%">
                  <input type="text" class="form-control input-sm" name="tnama_rm_cr" id="tnama_rm_cr" >
                </td>
                <td width="7%">&nbsp;</td>
              ...
              <tr>
                ...
                <td>:</td>
                <td>
                  <input type="text" class="form-control input-sm" name="talamat_rm_cr" id="talamat_rm_cr" >
                </td>
                <td>&nbsp;</td>
                <td></td>
                <td></td>
       ...
       <div class="tab-pane fade" id="tab_detail">
          ...
          <!--  <div class="alert alert-warning"> -->
            ...
            <tr>
              <td>Nama RM/Restoran</td>
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm" id="tnama_rm" name="tnama_rm">
              </td>
            </tr>
            <tr>
              <td>Alamat</td>
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm" id="talamat_rm" name="talamat_rm">
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm" id="tno_ijin" name="tno_ijin">
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>:</td>
              <td>
                <input type="text" class="form-control input-sm size_kecil" id="tjml_penj" name="tjml_penj">
              </td>
            </tr>

...
class mdl_dt_kesling extends CI_Model {
...
function _get_datatables_pencarian($thn,$bln,$desa,$term=''){
    ...
    $this->db->select("gizi.id_kesling,gizi.thn_kesling,gizi.bln_kesling,desa.nama_desa,kel.nama_kel");
    $this->db->from('dt_kesling gizi');
    $this->db->join('m_desa desa','desa.kd_desa=gizi.kd_desa');
    $this->db->join('m_kel kel','desa.kd_kel=kel.kd_kel');

        if ($thn !='') {
			$this->db->where('gizi.thn_kesling',$thn);        	
        }

		if ($bln!='') {
			$this->db->where('gizi.bln_kesling',$bln);        	
        }        
    ...
    if ($term !='') {
    	...
    	$this->db->like('gizi.thn_kesling',$term);
    	$this->db->or_like('gizi.bln_kesling',$term);
    	$this->db->or_like('desa.nama_desa',$term);

    }

    $this->db->order_by("gizi.thn_kesling,gizi.bln_kesling","desc");
...
public function get_by_id_pencarian($id)
    {
        ...
        $this->db->where('gizi.id_kesling',$id);
        $query = $this->db->get();
...
public function save($data)
    {
            $this->db->insert("dt_kesling", $data);
            return $this->db->insert_id();   
    }

public function update($where, $data)
    {
        $this->db->update("dt_kesling", $data, $where);
        return $this->db->affected_rows();
    }

public function delete($id)
    {
        $this->db->where('id_kesling', $id);
        $this->db->delete("dt_kesling");
    }

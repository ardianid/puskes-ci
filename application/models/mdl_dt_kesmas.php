...
class mdl_dt_kesmas extends CI_Model {
...
function _get_datatables_pencarian($thn,$bln,$desa,$term=''){
    ...
    $this->db->select("gizi.id_kesmas,gizi.thn_kesmas,gizi.bln_kesmas,desa.nama_desa,kel.nama_kel");
    $this->db->from('dt_kesmas gizi');
    $this->db->join('m_desa desa','desa.kd_desa=gizi.kd_desa');
    $this->db->join('m_kel kel','desa.kd_kel=kel.kd_kel');

        if ($thn !='') {
			$this->db->where('gizi.thn_kesmas',$thn);        	
        }

		if ($bln!='') {
			$this->db->where('gizi.bln_kesmas',$bln);        	
        }        
    ...
    if ($term !='') {
    	...
    	$this->db->like('gizi.thn_kesmas',$term);
    	$this->db->or_like('gizi.bln_kesmas',$term);
    	$this->db->or_like('desa.nama_desa',$term);

    }

    $this->db->order_by("gizi.thn_kesmas,gizi.bln_kesmas","desc");
...
public function get_by_id_pencarian($id)
    {
        ...
        $this->db->where('gizi.id_kesmas',$id);
        $query = $this->db->get();
...
public function save($data)
    {
            $this->db->insert("dt_kesmas", $data);
            return $this->db->insert_id();   
    }

public function update($where, $data)
    {
        $this->db->update("dt_kesmas", $data, $where);
        return $this->db->affected_rows();
    }

public function delete($id)
    {
        $this->db->where('id_kesmas', $id);
        $this->db->delete("dt_kesmas");
    }

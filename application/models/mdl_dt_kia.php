...
class mdl_dt_kia extends CI_Model {
...
function _get_datatables_pencarian($thn,$bln,$desa,$term=''){
    ...
    $this->db->select("gizi.id_kia,gizi.thn_kia,gizi.bln_kia,desa.nama_desa,kel.nama_kel");
    $this->db->from('dt_kia gizi');
    $this->db->join('m_desa desa','desa.kd_desa=gizi.kd_desa');
    $this->db->join('m_kel kel','desa.kd_kel=kel.kd_kel');

        if ($thn !='') {
			$this->db->where('gizi.thn_kia',$thn);        	
        }

		if ($bln!='') {
			$this->db->where('gizi.bln_kia',$bln);        	
        }        
    ...
    if ($term !='') {
    	...
    	$this->db->like('gizi.thn_kia',$term);
    	$this->db->or_like('gizi.bln_kia',$term);
    	$this->db->or_like('desa.nama_desa',$term);

    }

    $this->db->order_by("gizi.thn_kia,gizi.bln_kia","desc");
...
public function save($data)
    {
            $this->db->insert("dt_kia", $data);
            return $this->db->insert_id();   
    }

public function update($where, $data)
    {
        $this->db->update("dt_kia", $data, $where);
        return $this->db->affected_rows();
    }

public function delete($id)
    {
        $this->db->where('id_kia', $id);
        $this->db->delete("dt_kia");
    }

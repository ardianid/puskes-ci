...
class mdl_dt_imunisasi extends CI_Model {
...
function _get_datatables_pencarian($thn,$bln,$desa,$term=''){
    ...
    $this->db->select("gizi.id_imun,gizi.thn_imun,gizi.bln_imun,desa.nama_desa,kel.nama_kel");
    $this->db->from('dt_imunisasi gizi');
    $this->db->join('m_desa desa','desa.kd_desa=gizi.kd_desa');
    $this->db->join('m_kel kel','desa.kd_kel=kel.kd_kel');

        if ($thn !='') {
			$this->db->where('gizi.thn_imun',$thn);        	
        }

		if ($bln!='') {
			$this->db->where('gizi.bln_imun',$bln);        	
        }        
    ...
    if ($term !='') {
    	...
    	$this->db->like('gizi.thn_imun',$term);
    	$this->db->or_like('gizi.bln_imun',$term);
    	$this->db->or_like('desa.nama_desa',$term);

    }

    $this->db->order_by("gizi.thn_imun,gizi.bln_imun","desc");
...
public function save($data)
    {
            $this->db->insert("dt_imunisasi", $data);
            return $this->db->insert_id();   
    }

public function update($where, $data)
    {
        $this->db->update("dt_imunisasi", $data, $where);
        return $this->db->affected_rows();
    }

public function delete($id)
    {
        $this->db->where('id_imun', $id);
        $this->db->delete("dt_imunisasi");
    }
